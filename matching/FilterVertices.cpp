...
bool FilterVertices::EFilter(Graph *data_graph, Graph *query_graph, ui **&candidates, ui *&candidates_count, int top_s)
{
...
bool FilterVertices::LDFFilter(Graph *data_graph, Graph *query_graph, ui **&candidates, ui *&candidates_count, bool isEigenCheck, int top_s)
{
    if (isEigenCheck)
    {
        allocateBuffer(data_graph, query_graph, candidates, candidates_count);
        ...
        for (ui i = 0; i < query_graph->getVerticesCount(); ++i)
        {
            ...
            for (ui j = 0; j < data_vertex_num; ++j)
            {
                ...
                if (data_graph->getVertexDegree(data_vertex) >= degree)
                {
                    ...
                    for (ui e = 0; e < top_s; e++)
                    {
                        if (datagraph_eigenvalue.row(data_vertex)[e] > querygraph_eigenvalue.row(i)[e] || abs(datagraph_eigenvalue.row(data_vertex)[e] - querygraph_eigenvalue.row(i)[e]) < 0.001)
                        {
                            top_s_check = true;
                        }
                        else
...
bool FilterVertices::TSOFilter(Graph *data_graph, Graph *query_graph, ui **&candidates, ui *&candidates_count,
...
bool FilterVertices::CECIFilter(Graph *data_graph, Graph *query_graph, ui **&candidates, ui *&candidates_count,
...
bool FilterVertices::LDFWrapper(Graph *data_graph, Graph *query_graph, ui **&candidates, ui *&candidates_count,
                                ui *&order, TreeNode *&tree, std::vector<std::unordered_map<VertexID, std::vector<VertexID>>> &TE_Candidates,
                                std::vector<std::vector<std::unordered_map<VertexID, std::vector<VertexID>>>> &NTE_Candidates, bool isEigenCheck, int top_s)
{
...
bool FilterVertices::NLFWrapper(Graph *data_graph, Graph *query_graph, ui **&candidates, ui *&candidates_count,
                                ui *&order, TreeNode *&tree, std::vector<std::unordered_map<VertexID, std::vector<VertexID>>> &TE_Candidates,
                                std::vector<std::vector<std::unordered_map<VertexID, std::vector<VertexID>>>> &NTE_Candidates, bool isEigenCheck, int top_s)
{
...
bool FilterVertices::GQLWrapper(Graph *data_graph, Graph *query_graph, ui **&candidates, ui *&candidates_count,
                                ui *&order, TreeNode *&tree, std::vector<std::unordered_map<VertexID, std::vector<VertexID>>> &TE_Candidates,
                                std::vector<std::vector<std::unordered_map<VertexID, std::vector<VertexID>>>> &NTE_Candidates, bool isEigenCheck, int top_s)
{
...
bool FilterVertices::TSOWrapper(Graph *data_graph, Graph *query_graph, ui **&candidates, ui *&candidates_count,
                                ui *&order, TreeNode *&tree, std::vector<std::unordered_map<VertexID, std::vector<VertexID>>> &TE_Candidates,
                                std::vector<std::vector<std::unordered_map<VertexID, std::vector<VertexID>>>> &NTE_Candidates, bool isEigenCheck, int top_s)
{
...
bool FilterVertices::CFLWrapper(Graph *data_graph, Graph *query_graph, ui **&candidates, ui *&candidates_count,
                                ui *&order, TreeNode *&tree, std::vector<std::unordered_map<VertexID, std::vector<VertexID>>> &TE_Candidates,
                                std::vector<std::vector<std::unordered_map<VertexID, std::vector<VertexID>>>> &NTE_Candidates, bool isEigenCheck, int top_s)
{
...
bool FilterVertices::DPisoWrapper(Graph *data_graph, Graph *query_graph, ui **&candidates, ui *&candidates_count,
                                  ui *&order, TreeNode *&tree, std::vector<std::unordered_map<VertexID, std::vector<VertexID>>> &TE_Candidates,
                                  std::vector<std::vector<std::unordered_map<VertexID, std::vector<VertexID>>>> &NTE_Candidates, bool isEigenCheck, int top_s)
{
...
void FilterVertices::computeCandidateWithNLF(Graph *data_graph, Graph *query_graph, VertexID query_vertex,
            ...
            if (data_vertex_nlf->size() >= query_vertex_nlf->size())
            {
                ...
                if (is_valid)
                {
                    ...
                    if (false)
                    {
                        ...
                        for (ui e = 0; e < top_s; e++)
                        {
                            if (datagraph_eigen.row(data_vertex)[e] > querygraph_eigen.row(query_vertex)[e] || abs(datagraph_eigen.row(data_vertex)[e] - querygraph_eigen.row(query_vertex)[e]) < 0.001)
                            {
                                top_s_check = true;
                            }
                            else
