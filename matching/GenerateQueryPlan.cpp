...
#include <limits>
VertexID GenerateQueryPlan::selectGQLStartVertexF(const Graph *query_graph, float candidates_count[])
{
    /**
...
void GenerateQueryPlan::generateGQLQueryPlanN(const Graph *data_graph, const Graph *query_graph, float weight[],
                                              ui *&order, ui *&pivot)
{
    /**
    ...
    VertexID start_vertex = selectGQLStartVertexF(query_graph, weight);
    ...
    for (ui i = 1; i < query_graph->getVerticesCount(); ++i)
    {
        ...
        for (ui j = 0; j < query_graph->getVerticesCount(); ++j)
        {
            ...
            {
                if (weight[cur_vertex] < min_value)
                {
                    min_value = weight[cur_vertex];
                    next_vertex = cur_vertex;
                }
                else if (weight[cur_vertex] == min_value && query_graph->getVertexDegree(cur_vertex) > query_graph->getVertexDegree(next_vertex))
                {
                    next_vertex = cur_vertex;
                }
            }
            ...
            if (visited_vertices[begin_vertex])
            {
                ...
                for (ui j = 0; j < nbrs_cnt; ++j)
                {
                    ...
                    if (!visited_vertices[end_vertex])
                    {
                        ...
                        if (cur_value < min_value || (cur_value == min_value && adjacent_selected_vertices < max_adjacent_selected_vertices) || (cur_value == min_value && adjacent_selected_vertices == max_adjacent_selected_vertices && cur_degree > max_degree))
                        {
...
std::pair<VertexID, VertexID> GenerateQueryPlan::selectQSIStartEdge(const Graph *query_graph, Edges ***edge_matrix)
{
    ...
    for (ui i = 0; i < query_graph->getVerticesCount(); ++i)
    {
        ...
        for (ui j = 0; j < nbrs_cnt; ++j)
        {
            ...
            if (cur_value < min_value || (cur_value == min_value && (cur_degree_sum < min_degree_sum)))
            {
                min_value = cur_value;
                min_degree_sum = cur_degree_sum;

                start_edge = query_graph->getVertexDegree(begin_vertex) < query_graph->getVertexDegree(end_vertex) ? std::make_pair(end_vertex, begin_vertex) : std::make_pair(begin_vertex, end_vertex);
            }
        }
    }
...
void GenerateQueryPlan::generateVF2PPQueryPlan(const Graph *data_graph, const Graph *query_graph, ui *&order, ui *&pivot)
{
    ui query_vertices_num = query_graph->getVerticesCount();
    order = new ui[query_vertices_num];
    pivot = new ui[query_vertices_num];
